...
class Database:
  ...
  def _commit(self):
    """Commit immediately unless an outer entry transaction is active."""
    if self._transaction_depth == 0:
      self._entry_conn.commit()

  def _rollback(self):
    """Rollback immediately unless an outer entry transaction owns rollback."""
    if self._transaction_depth == 0:
      self._entry_conn.rollback()

  @contextmanager
  def entry_transaction(self):
    """
    Wrap one dictionary-entry update in a single SQLite transaction.

    Atomic CRUD helpers call ``_commit()``/``_rollback()`` instead of touching the
    connection directly, so their writes are committed only when the outer
    entry-level transaction succeeds.
    """
    outermost = self._transaction_depth == 0
    savepoint_name = None
    if outermost:
      self._entry_conn.execute("BEGIN")
    else:
      self._savepoint_counter += 1
      savepoint_name = f"entry_nested_{self._savepoint_counter}"
      self._entry_conn.execute(f"SAVEPOINT {savepoint_name}")
    ...
    try:
      ...
      if outermost:
        ...
        self._entry_conn.execute(f"RELEASE SAVEPOINT {savepoint_name}")
    except Exception:
      self._transaction_depth -= 1
      if outermost:
        self._entry_conn.rollback()
      else:
        self._entry_conn.execute(f"ROLLBACK TO SAVEPOINT {savepoint_name}")
        self._entry_conn.execute(f"RELEASE SAVEPOINT {savepoint_name}")
      raise
  ...
  def get_entities_index(self) -> List[str]:
    cursor = self._entry_conn.cursor()
    try:
      cursor.execute(f"SELECT id, title, type FROM Entities ORDER BY id")
      rows = cursor.fetchall()
      entries = [
        f"ID={entity_id} | title={title} | type={entity_type or '未知'}"
        for (entity_id, title, entity_type) in rows
      ]
      return entries
    except Exception as e:
      self._rollback()
  ...
  def get_entities_index_filtered(self, ids) -> List[str]:
    """按给定 id 集合返回实体索引行，用于上下文压缩时只发候选实体。

    Args:
      ids: 可迭代的 entity_id；空集合直接返回 []。
    Returns:
      与 get_entities_index 相同的字符串列表（不存在的 id 自动忽略）。
    """
    id_list = list({int(x) for x in ids if x is not None})
    if not id_list:
      return []
    cursor = self._entry_conn.cursor()
    try:
      ...
      cursor.execute(
        f"SELECT id, title, type FROM Entities WHERE id IN ({placeholders}) ORDER BY id",
        id_list,
      )
      rows = cursor.fetchall()
      return [
  ...
  def get_entities_id_by_title_pattern(self, pattern: str, limit: int = 8) -> List[int]:
    """按 title 子串/前缀宽松匹配返回 entity_id，用于候选实体召回扩展。

    Args:
      pattern: title 子串（不含通配符，由本方法负责包成 %pattern%）；空串返回 []。
      limit: 单次召回上限，按 id ASC 截断。
    Returns:
      命中实体的 id 列表（可能为空）。
    """
    ...
    try:
      cursor.execute(
        "SELECT id FROM Entities WHERE title LIKE ? ORDER BY id LIMIT ?",
        (f"%{pattern}%", int(limit)),
      )
    ...
    try:
      cursor.execute(
        """SELECT id FROM Relationships
           WHERE subject_id = ?
             AND object_id = ?
             AND COALESCE(relation_type, '') = COALESCE(?, '')
             AND COALESCE(staff_quota, -1) = COALESCE(?, -1)
             AND COALESCE(staff_type, '') = COALESCE(?, '')
           ORDER BY id
           LIMIT 1""",
  ...
  def find_relationship(
    ...
    """Find an existing relationship with the same semantic key."""
    cursor = self._entry_conn.cursor()
    try:
      cursor.execute(
    ...
    try:
      cursor.execute(
        """SELECT id FROM Citations
           WHERE target_table = ?
             AND target_id = ?
             AND COALESCE(citation, '') = COALESCE(?, '')
             AND COALESCE(quotation, '') = COALESCE(?, '')
