...
核验只把当前时间点 ID 和引文行 ID 发送给本地后端；后端从正式只读库重新读取事件和逐字引文。结果分为“支持”“相关但证据不足”“直接冲突”“完全无关”四级：前三类必须返回并高亮逐字相关片段，只有完全无关允许没有片段，并以更高风险显示。调用模型前不预先用整段 quotation 标记词条原文；模型生成后才按返回片段同步标记原文引文与词条原文。核验结果写入独立的 `*.evidence-reviews.db` 旁路缓存，刷新页面或重启服务后会自动恢复；正式辞典库和结构化结果库保持只读。结果仅表示本条引文与该结构化事件的证据关系，不代表史实确认。
