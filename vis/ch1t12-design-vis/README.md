...
核验只把当前时间点 ID 和引文行 ID 发送给本地后端；后端从正式只读库重新读取事件和逐字引文。结果分为“支持”“相关但证据不足”“直接冲突”“完全无关”四级：前三类必须返回并高亮逐字相关片段，只有完全无关允许没有片段，并以更高风险显示。结果仅表示本条引文与该结构化事件的证据关系，不代表史实确认，也不会写入数据库或浏览器持久存储。
