...
- `public/data/song-bureaucracy.json` 仅保留为离线快照，不再是 v2 运行时数据源。
...
后端通过只读 SQLite 连接实时装配前端数据。修改工作库并提交事务后，页面会在约 1.5 秒内检测到主数据库或 WAL 变化并刷新；新增或修改 `Timepoints.time` 时会按最新中文时间即时标准化，不依赖旧 `NormalizedTimes`。
