...
- 时间选择沿用 v2 `SongTimeline.vue` 的交互：单击时间轴选择单年，拖动空白区域新建范围，拖动既有范围或两端调整选择；本界面使用原 SVG 的三角指针、竖向虚线与贴合年轴的细实线呈现所选区间。点击“× 取消选择”会清除指针和选区，恢复全宋时段。机构树、编制、详情与时间点按所选区间同步过滤，年代未明的数据保留。
