...
| 底部 朝代/年份 两行时间线 + 右下角"编码信息"图例（机构级别、官职类分类、数据来源） | `SongTimelineBar.vue`；拖动年份会同时过滤两种视图和右侧详情 |
| 选中机构详情：下级机构、编制隶属官职、时间点 + 引用（Citations）、辞典词条 | `DetailPanel.vue` |
| 显示官职开关 | 顶栏复选框；未勾选时选中单个机构也会展出其官职面板（对应"单个浮动展开"） |
| 第二张 SVG 的编制构成画板 | `CompositionView.vue` 用 D3 treemap 动态组织机构及编制文字，和层级视图共享对象、年份与证据 |
...
src/
  ...
  components/
    ...
    CompositionView.vue   # d3 编制构成视图（第二张 SVG 画板）
...
- 时间筛选使用项目统一的宋代纪年标准化器；无明确年代的数据保留显示，避免把“时间未明”误判为“不存在”。
